package com.vengteshm.coronavirustracker.services;
...
public class CoronaVirusDataService {
    ...
    @PostConstruct
    @Scheduled(cron = "* * 1 * * *") // Runs every second, spring creates proxy to run this method
    public void fetchCoronaVirusData() throws IOException, InterruptedException {
        List<LocationStats> newStats = new ArrayList<>(); // For concurrency reasons
        ...
        for (CSVRecord record : records) {
            ...
            // Every day is a column, so it keeps on adding day by day
